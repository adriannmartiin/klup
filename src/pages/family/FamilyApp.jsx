// ============================================================
// KLUP — Family Panel (responsive, multi-athlete, join groups)
// ============================================================
...
// Mock family: 3 athletes in different sports (demo scenario)
const MY_ATHLETES = [
...
const VIEWS = [
  ...
  { id:'info',    label:'Grupo',   icon:<Icon.Users/> },
      ...
      <aside className="app-sidebar">
        ...
        <nav className="sidebar-nav">
          ...
          {MY_ATHLETES.map(a => {
            ...
            return (
              <button key={a.id}
                className={`sidebar-nav-item ${activeAthleteId===a.id?'active':''}`}
                onClick={() => { setActiveAthleteId(a.id); setView('feed'); }}>
                <div className="avatar avatar-sm"
                  style={{ background:`${s?.color}20`, color:s?.color, fontSize:9 }}>
                  {a.avatar}
                </div>
          ...
          {VIEWS.map(v => (
            <button key={v.id}
              className={`sidebar-nav-item ${view===v.id?'active':''}`}
        ...
        <div className="sidebar-footer">
          <div className="sidebar-user">
            <div className="avatar avatar-md" style={{ background:'#f3e8ff', color:'#9333ea' }}>
              {user?.avatar||'AG'}
            </div>
            <div className="sidebar-user-info">
              <div className="sidebar-user-name">{user?.name}</div>
              <div className="sidebar-user-role">{MY_ATHLETES.length} atletá{MY_ATHLETES.length>1?'s':''}</div>
            </div>
          </div>
      ...
      <div className="app-main">
        ...
        <header className="page-header mobile-only">
          ...
          <div style={{ display:'flex', gap:8, marginTop:10, overflowX:'auto', paddingBottom:2 }}>
            {MY_ATHLETES.map(a => {
              ...
              return (
                <button key={a.id} onClick={() => { setActiveAthleteId(a.id); setView('feed'); }}
                  ...
                  <div className="avatar avatar-sm" style={{ background:s?.color, color:'white', fontSize:9 }}>
                    {a.avatar}
                  </div>
            ...
            <button onClick={() => setShowJoinModal(true)}
              ...
              <svg width="12" height="12"><Icon.Plus/></svg>
              Añadir
            </button>
          </div>
        </header>
        ...
        <div className="desktop-header">
          ...
          <span className="desktop-header-title">
            {athlete.name} — {sport?.name} · {group?.name}
          </span>
        ...
        <div className="page-content">
          ...
          {view === 'feed' && (
            <>
              {feedPosts.length === 0 ? (
                <EmptyState icon={<Icon.Home/>} title="Sin publicaciones"
                  text="El entrenador todavía no ha publicado nada en este grupo"/>
              ) : feedPosts.map(post => (
                <FeedPost key={post.id} post={post} sport={sport} group={group}/>
              ))}
            </>
          )}
            ...
            <>
              <div className="card" style={{ border:`1.5px solid ${sport?.color}30`, marginBottom:16 }}>
                ...
                <div style={{ fontSize:13, color:'var(--muted)', lineHeight:1.8 }}>
                  ...
                  <strong style={{ color:'var(--text)' }}>Capacidad:</strong> {group?.count}/{group?.maxCapacity} atletas<br/>
              ...
              {coach && (
                <>
                  ...
                  <div className="card" style={{ display:'flex', alignItems:'center', gap:12 }}>
                    <div className="avatar avatar-lg" style={{ background:'#dbeafe', color:'#2563eb' }}>
                      {coach.avatar}
                    </div>
                    <div style={{ flex:1 }}>
                    ...
                    <button className="btn btn-info btn-sm">
                      <svg width="13" height="13"><Icon.Megaphone/></svg>
                      Mensaje
                    </button>
                  </div>
                </>
              ...
              <div className="section-label" style={{ marginTop:8 }}>Compañeros del grupo</div>
              <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(70px,1fr))', gap:8 }}>
                {ATHLETES.filter(a => a.groupId === athlete.groupId && a.id !== athlete.id).map(a => (
                  <div key={a.id} style={{ textAlign:'center', padding:'8px 4px',
                    background:'var(--surface)', borderRadius:'var(--r-md)', border:'1px solid var(--border)' }}>
                    <div className="avatar avatar-sm" style={{ background:`${sport?.color}20`,
                      color:sport?.color, margin:'0 auto 4px' }}>{a.avatar}</div>
          ...
          {view === 'informes' && (
            <>
              ...
              {healthRecord && (
                <>
                  <div className="section-label" style={{ marginTop:8 }}>Ficha médica</div>
                  <div className="health-card">
                    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:6 }}>
                      <span style={{ fontSize:13, fontWeight:700, color:'#c2410c' }}>Info sanitaria</span>
                    ...
                    {showHealthDetails && (
                      <div style={{ fontSize:12, color:'#92400e', lineHeight:1.8 }}>
                        ...
                        <div style={{ borderTop:'1px solid #fed7aa', marginTop:8, paddingTop:8 }}>
              ...
              <div className="section-label" style={{ marginTop:8 }}>Observaciones del entrenador</div>
              ...
              {myObs.length === 0 ? (
                <EmptyState icon={<Icon.File/>} title="Sin observaciones"
                  text="El entrenador todavía no ha añadido observaciones"/>
              ) : myObs.map(obs => {
                ...
                return (
                  <div key={n.id} style={{ display:'flex', gap:10, padding:'12px 14px',
                    ...
                    {!n.read && <div style={{ width:8, height:8, borderRadius:'50%',
                      background:colors[n.type], flexShrink:0, marginTop:4 }}/>}
                  </div>
                );
              })}
            </>
            ...
            <>
              ...
              <div className="join-card" onClick={() => setShowJoinModal(true)} style={{ marginBottom:16 }}>
                ...
                <div style={{ fontSize:12, color:'var(--muted)', marginTop:3 }}>
                  Añade un hijo/a a una extraescolar
                </div>
              </div>
              ...
              <div className="card">
                ...
                <div style={{ fontSize:13, color:'var(--muted)', marginBottom:8 }}>
                  <strong style={{ color:'var(--text)' }}>Consentimiento foto:</strong>{' '}
                  <span style={{ color:'var(--presente)', fontWeight:700 }}>Autorizado</span>
                </div>
                <button style={{ fontSize:13, color:'var(--blue)', fontWeight:600, background:'none',
...
function FeedPost({ post, sport, group }) {
  return (
        ...
        {step === 0 && (
          <>
            ...
            <button className="btn btn-primary btn-full" style={{ height:46 }}
              onClick={() => setStep(1)} disabled={!athleteName.trim() || !course}>
              Continuar
            </button>
          </>
        )}
          ...
          <>
            ...
            <div style={{ display:'flex', gap:8, marginTop:8 }}>
              ...
              <button className="btn btn-primary" style={{ flex:2 }}
                onClick={() => setSent(true)} disabled={!sportId || !groupId}>
                Enviar solicitud
              </button>
            </div>
          </>
